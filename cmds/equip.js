...
module.exports.run = async (bot,message,args) => {
	try{
        ...
        if (args[0] == "role" || args[0] == "roles") {
			...
			let alredyEquiped = new Discord.MessageEmbed()
			.setColor(userPLine)
			.setTitle("⸝⸝ ♡₊˚ Инвентарь ролей◞")
			.setDescription(`Роль уже надета!`)
			.setFooter(`${rUser.username}`, `${rUser.displayAvatarURL({dynamic: true})}`)
			.setTimestamp();
			...
			if (userRoles.find(x => x.index == userIndex)) {
				...
				if (member.roles.cache.has(eqRoleId)) return message.channel.send(alredyEquiped);
				member.roles.add(eqRoleId);

				let Equiped = new Discord.MessageEmbed()
				.setColor(userPLine)
				.setTitle("⸝⸝ ♡₊˚ Инвентарь ролей◞")
				.setDescription(`Роль <@&${eqRoleId}> была надета.`)
				.setFooter(`${rUser.username}`, `${rUser.displayAvatarURL({dynamic: true})}`)
				.setTimestamp();

				message.channel.send(Equiped);
			} else {
			...
			.setDescription(`Пак уже в профиле!`)
			.setFooter(`${rUser.username}`, `${rUser.displayAvatarURL({dynamic: true})}`)
			.setTimestamp();
			...
			if (userEmoji.find(x => x.index == userIndex)) {
				if (eqEmoji == userIndex) return message.channel.send(alredyEquiped);
				...
				.setColor(userPLine)
				.setTitle("⸝⸝ ♡₊˚ Инвентарь ролей◞")
				.setDescription(`Пак эмоджи <${emojiName}> был успешно установлен в Ваш профиль.`)
				.setFooter(`${rUser.username}`, `${rUser.displayAvatarURL({dynamic: true})}`)
				.setTimestamp();
